...
$baseline = "> 基线：$generatedDate，``$branch`` 分支。当前源码包含 $pageCount 个 ``page.tsx`` 页面入口、$apiCount 个 API Route 文件和 $tableCount 张 PostgreSQL 表。接口逐项说明见 [VOZEB-PRO 接口索引](VOZEB-PRO-接口索引.md)，发布操作见 [VOZEB-PRO 更新与部署流程](VOZEB-PRO-更新部署流程.md)。"
$baselineMatch = [regex]::Match($map, "(?m)^> (?:集成)?基线：.*$")
if (-not $baselineMatch.Success) {
    throw "未找到 VOZEB-PRO-开发地图.md 的基线行，拒绝静默写入。"
}
$updated = [regex]::Replace($map, "(?m)^> (?:集成)?基线：.*$", [System.Text.RegularExpressions.MatchEvaluator]{ param($match) $baseline }, 1)
