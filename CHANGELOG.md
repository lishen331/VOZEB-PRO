...
- [权限] 增加 PostgreSQL RBAC 角色目录与用户角色绑定，平台后台新增角色管理和管理员管理；默认平台超管、平台管理员、学校管理员、老师、学生和普通用户角色及受保护管理员不可停用或删除。
- [权限] 用户运营仅管理普通用户，学校成员保留学校归属并改由学校流程管理身份与状态；学校成员查询和积分调整改为显式产教/财务职责校验，阻止通过直连接口越权。

- [Agent] 修复规划重试身份、Skill参考校验和多模型部分结果同步，增加待确认原任务检查入口；正式模型候选排除练习专用渠道。线上回归待完成。

- [Agent] 浏览生成参数不再暗中强制媒体类型；图片/视频规划读取真实媒体内容并按协议传递，私有素材读取失败明确中止。线上多模态与全能力回归进行中。

- [IP 库] 重构为 `IP -> 子 IP -> 内容项`：删除 IP 版本、发布和学校二次开启，保存立即生效；子 IP 支持独立内容、封面、授权、下载与引用，多校/独家授权在子 IP 粒度校验冲突。
- [IP 库] 后台列表增加详情、停用和删除，详情页改为全页编辑器；停用后学校管理员仍可查看授权记录，老师和学生不可访问。旧 IP 业务数据由一次性迁移清空，已登记文件进入可重试的清理队列。
- [短剧] Phase 4 收敛运行时配置：九套提示词使用全局模板，模型/渠道与任务继续使用平台权威链路；历史兼容配置不再伪装为生效设置，假成功的 AI 测试接口改为明确 410。
- [短剧] Phase 4 进一步收口历史配置：AI、业务场景和 SD2 兼容 Tab 改为只读，生成设置显示平台全局并发别名；对应旧写入、上传和删除接口统一返回 410，避免“保存成功但运行不生效”。
- [短剧] 提示词后台保存改为全局键原子 UPSERT，避免多管理员首次编辑同一模板时发生竞态冲突。
- [工程] 增加只读部署健康冒烟命令 `pnpm run smoke:deployment`，可分别检查应用存活和 Generation Worker 就绪状态。
- [短剧] 新增按剧集隔离的专属画布，复用当前 Canvas 创作能力并保留剧集/分镜定位；普通画布、发布来源、删除清理和学校商单计费均按短剧归属隔离。
- [短剧] 修复分镜视频上游失败被误当成媒体地址的问题；保留原任务供状态检查，避免前端持续转圈、重复提交和重复扣费，并支持标准 `/videos/:task_id/content` 结果地址。
- [测试] Phase 4 增加短剧工作台与按剧集 Canvas 的真实 Playwright 回归，覆盖分镜定位/返回、剧集切换及 390px/430px 响应式无溢出；修正协作邀请兼容路由的 Next.js `dynamic` 配置声明。
