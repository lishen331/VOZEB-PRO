...
> 集成后路由与方法数量以当前源码为准，以下两份基线统计保留供历史对照。
> 生成日期：2026-09-02。枚举来源仅为 `web/src/app/api/**/route.ts`；当前共 **299** 个 Route 文件。每个文件一行，多种 HTTP 方法合并显示。
> 生成日期：2026-09-03。枚举来源仅为 `web/src/app/api/**/route.ts`；当前共 **296** 个 Route 文件。每个文件一行，多种 HTTP 方法合并显示。
