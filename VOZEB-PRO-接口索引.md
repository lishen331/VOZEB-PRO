...
> 生成日期：2026-09-06。枚举来源仅为 `web/src/app/api/**/route.ts`；当前共 **332** 个 Route 文件。每个文件一行，多种 HTTP 方法合并显示。
...
- Route 文件：**332**
- 方法出现次数：DELETE 52、GET 182、HEAD 6、PATCH 54、POST 177、PUT 11
- 一级域：`admin` 114、`agent` 8、`ai` 1、`announcements` 1、`audio-tasks` 2、`auth` 13、`billing` 11、`canvas` 4、`cdk` 1、`check-in` 1、`community` 1、`create` 1、`creative` 6、`debug` 1、`drama` 12、`drama-lab` 43、`generation-log-assets` 1、`generation-logs` 1、`generation-webhooks` 1、`health` 2、`image-tasks` 2、`install` 2、`ip-library` 5、`library-assets` 2、`maintenance` 6、`media-assets` 1、`media-proxy` 1、`my-prompts` 2、`notifications` 3、`points` 1、`practice` 5、`prompts` 1、`public` 16、`reference-assets` 2、`referrals` 1、`school` 28、`site-icon` 1、`teaching` 15、`text-tasks` 2、`video-generation-tasks` 2、`video-tasks` 2、`works` 7
...
| POST | `/api/drama-lab/projects/[id]/extract-storyboards` | 用户 | [route.ts](web/src/app/api/drama-lab/projects/[id]/extract-storyboards/route.ts) | [drama-lab-collaboration-service](web/src/lib/server/drama-lab-collaboration-service.ts)<br>[drama-lab-workflow-task-service](web/src/lib/server/drama-lab-workflow-task-service.ts) | PostgreSQL | drama-lab / 项目 / 单项 / extract-storyboards：提交/执行 |
...
| POST | `/api/drama-lab/projects/[id]/import-novel` | 用户 | [route.ts](web/src/app/api/drama-lab/projects/[id]/import-novel/route.ts) | [drama-lab-collaboration-service](web/src/lib/server/drama-lab-collaboration-service.ts)<br>[drama-lab-novel-import-service](web/src/lib/server/drama-lab-novel-import-service.ts)<br>[drama-novel-text-decoder](web/src/lib/drama-novel-text-decoder.ts) | PostgreSQL | drama-lab / 项目 / 单项 / import-novel：提交/执行 |
...
| GET | `/api/drama-lab/projects/[id]/tasks` | 混合 | [route.ts](web/src/app/api/drama-lab/projects/[id]/tasks/route.ts) | [drama-lab-collaboration-service](web/src/lib/server/drama-lab-collaboration-service.ts)<br>[drama-lab-task-service](web/src/lib/server/drama-lab-task-service.ts) | PostgreSQL | drama-lab / 项目 / 单项 / 任务：查询 |
| POST | `/api/drama-lab/projects/[id]/tasks/[taskId]/cancel` | 混合 | [route.ts](web/src/app/api/drama-lab/projects/[id]/tasks/[taskId]/cancel/route.ts) | [drama-lab-collaboration-service](web/src/lib/server/drama-lab-collaboration-service.ts)<br>[drama-lab-task-service](web/src/lib/server/drama-lab-task-service.ts) | PostgreSQL | drama-lab / 项目 / 单项 / 任务 / 指定任务 / 取消：提交/执行 |
