...
> 生成日期：2026-09-14。枚举来源仅为 `web/src/app/api/**/route.ts`；当前共 **371** 个 Route 文件。每个文件一行，多种 HTTP 方法合并显示。
...
| GET | `/api/practice/scripts/[id]/export` | 混合 | [route.ts](web/src/app/api/practice/scripts/[id]/export/route.ts) | [script-practice-service](web/src/lib/server/script-practice-service.ts)<br>[postgres](web/src/lib/server/database/postgres.ts)<br>[script-agent-repository](web/src/lib/server/database/script-agent-repository.ts) | PostgreSQL | practice / scripts / 单项 / 导出：查询 |
