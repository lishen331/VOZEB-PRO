...
| POST | `/api/practice/scripts/[id]/confirmations` | 混合 | [route.ts](web/src/app/api/practice/scripts/[id]/confirmations/route.ts) | [script-agent-run-service](web/src/lib/server/script-agent-run-service.ts)<br>[postgres](web/src/lib/server/database/postgres.ts)<br>[script-agent-repository](web/src/lib/server/database/script-agent-repository.ts) | PostgreSQL | practice / scripts / 单项 / confirmations：提交/执行 |
