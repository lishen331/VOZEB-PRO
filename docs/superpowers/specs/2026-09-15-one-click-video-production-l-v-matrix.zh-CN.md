...
| **必须迁移的业务逻辑** | **108** | **已覆盖 98，即 90.7%**（§13–§18 逐条核算） |

**结论（2026-09-17 更新）：主链路已覆盖 98/108（90.7%），分镜与资产链路具备真机验证条件；仍有 6 条 P2 缺口与 4 条刻意不复制项，不等于 1:1 完成。**
...
| 域 | 覆盖/总数 | 缺口 | 说明 |
|---|---|---|---|
| storyboards | 17/21 | 4（P2） | 3 条 stream 版本（非流式已迁移）、episode generate 端点 |
...
**合计：覆盖 98/108，刻意不复制 4，仍为缺口 6（全部 P2）。**
...
| `POST   /storyboards/:id/upscale` | P0 | 已迁移 |
