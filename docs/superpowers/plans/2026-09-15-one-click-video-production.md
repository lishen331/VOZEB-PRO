...
- 创作工坊是教学版；一键成片是商单生产版，两者同级、独立演进。
- 一键成片必须 1:1 复刻 L 的业务行为，不允许按教学版做简化迁移。
- V 只替换 UI 和平台基础设施，不替换 L 的业务语义、状态机、请求合同和异常处理。
- 每个任务必须先完成 L/V 差异矩阵和同输入回归证据，再进入实现。

## 产品定位修正

- 创作工坊是教学版；一键成片是商单生产版，两者同级。
- 一键成片严格 1:1 复刻 L 业务行为。
- V 只替换 UI 和平台基础设施，不替换 L 业务语义。
