...
import { buildImageReferencePromptText, buildImageTaskPrompt } from "@/lib/image-reference-prompt";
    ...
    const response = await imageSubmissionFetch(config, url, {
        ...
        body: JSON.stringify({
            ...buildGlobalAiOpcImageRequest(preset, {
                model: config.model,
                prompt: task.upstreamPrompt || buildImageTaskPrompt(task),
                quality,
                size: requestSize,
                ratio,
...
export async function runOpenAiImageTaskWithBase64Response(task: ImageTask, origin: string, publicOrigin: string, cookie: string, singleStep = false, billingVariant = "base64"): Promise<ImageTaskRunResult> {
    ...
    const response = await imageSubmissionFetch(config, url, {
        ...
        body: JSON.stringify({
            model: config.model,
            prompt: task.upstreamPrompt || withSystemPrompt(config, task.prompt),
            ...(config.outputMode === "layers" ? {} : { n: 1 }),
            ...(quality ? { quality } : {}),
            ...(requestSize ? { size: requestSize } : {}),
