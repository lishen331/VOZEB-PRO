import { recheckAgentRun } from "@/lib/server/agent-run-recheck";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; action: string }> }) {
    ...
    if (!status && action !== "retry" && action !== "cancel" && action !== "recheck") return NextResponse.json({ code: 400, data: null, msg: "不支持的 Agent 操作" }, { status: 400 });
    ...
    if (action === "recheck" && (run.status !== "paused" || run.cancellation)) return NextResponse.json({ code: 409, data: null, msg: "当前任务不能重新检查" }, { status: 409 });
    ...
    const limit = action === "resume" || action === "retry" || action === "recheck" ? (await getAuthSettings()).generationConcurrency.agent : 0;
    if (action === "cancel" && ["completed", "partial_success", "failed", "cancelled"].includes(run.status)) return NextResponse.json({ code: 409, data: null, msg: "当前任务无法取消" }, { status: 409 });
    if (action === "cancel") return cancelAgentRun(request, run);
    if (action !== "resume") abortAgentRun(run.id);
    const mutate = async () => ({
        updated:
            action === "recheck" || (action === "resume" && run.tasks.some((task) => task.status === "needs_review"))
                ? await recheckAgentRun(run)
                : action === "retry"
    ...
    let result: Awaited<ReturnType<typeof mutate>> | null;
    try {
        result = action === "resume" || action === "retry" || action === "recheck" ? await withGenerationConcurrencyLimit(run.userId, "agent", 10 * 60 * 1000, limit, mutate, run.id) : await mutate();
    } catch (error) {
        if (action !== "recheck" && action !== "resume") throw error;
        return NextResponse.json({ code: 409, data: null, msg: error instanceof Error ? error.message : "无法检查原任务" }, { status: 409 });
    }
    ...
    if ((action === "resume" || action === "retry" || action === "recheck") && updated.status !== "paused") {
