...
import { canvasProjectError, deleteDramaLabCanvasAssistantConversationsForUser } from "@/lib/server/canvas-project-service";
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        const result = await deleteDramaLabCanvasAssistantConversationsForUser(user.id, (await params).id, parsed.data?.conversationIds);
