...
import { canvasProjectError, getDramaLabCanvasProjectForUser, updateDramaLabCanvasProjectForUser } from "@/lib/server/canvas-project-service";
...
export async function GET(_request: Request, context: Context) {
    ...
    try {
        const project = await getDramaLabCanvasProjectForUser(user.id, (await context.params).id);
...
export async function PATCH(request: Request, context: Context) {
    ...
    try {
        ...
        const result = await updateDramaLabCanvasProjectForUser(user.id, (await context.params).id, parsed.data);
