...
function legacyShotToEpisodeShot(value: unknown, index: number, currentShots: unknown[]) {
    ...
    const textField = (key: string) => (typeof shot[key] === "string" ? shot[key] : typeof current?.[key] === "string" ? current[key] : undefined);
    const numberField = (key: string) => (Number.isFinite(Number(shot[key])) ? Number(shot[key]) : Number.isFinite(Number(current?.[key])) ? Number(current?.[key]) : undefined);
    const objectField = (key: string) => (shot[key] && typeof shot[key] === "object" && !Array.isArray(shot[key]) ? shot[key] : current?.[key]);
    return [
        {
            ...current,
            id,
            ...
            shotType: textField("shotType"),
            segmentIndex: numberField("segmentIndex"),
            segmentTitle: textField("segmentTitle"),
            atmosphere: textField("atmosphere"),
            lightingStyle: textField("lightingStyle"),
            depthOfField: textField("depthOfField"),
            creationMode: textField("creationMode"),
            universalSegmentText: textField("universalSegmentText"),
            polishedPrompt: textField("polishedPrompt"),
            angleH: textField("angleH"),
            angleV: textField("angleV"),
            angleS: textField("angleS"),
            location: textField("location"),
            time: textField("time"),
            action: textField("action"),
            result: textField("result"),
            emotion: textField("emotion"),
            emotionIntensity: numberField("emotionIntensity"),
            layoutDescription: textField("layoutDescription"),
            continuity: objectField("continuity"),
            ...
            clueIds: Array.isArray(shot.clueIds) ? shot.clueIds.filter((item): item is string => typeof item === "string") : Array.isArray(current?.clueIds) ? current.clueIds : [],
