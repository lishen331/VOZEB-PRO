...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try { const { id } = await params; return NextResponse.json({ code: 0, data: { invites: await listDramaLabInvites(user.id, id) }, msg: "OK" }); } catch (error) { return handle(error); }
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const invite = parsed.data.rotate === true ? await rotateDramaLabInvite(user.id, id, { expiresAt: typeof parsed.data.expiresAt === "string" ? parsed.data.expiresAt : undefined }) : await createDramaLabInvite(user.id, id, { expiresAt: typeof parsed.data.expiresAt === "string" ? parsed.data.expiresAt : undefined });
