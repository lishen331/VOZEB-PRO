...
import { removeDramaLabMember, setDramaLabMemberRole, transferDramaLabOwnership, DramaLabCollaborationError } from "@/lib/server/drama-lab-collaboration-service";
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string; userId: string }> }) {
    ...
    try { const { id, userId } = await params; return NextResponse.json({ code: 0, data: await removeDramaLabMember(user.id, id, userId), msg: "成员已移除" }); } catch (error) { return handle(error); }
}
