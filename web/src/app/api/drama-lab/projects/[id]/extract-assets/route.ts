...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const status = error instanceof FeatureModuleDisabledError ? 403 : error instanceof DramaLabAssetExtractionError || isDramaLabCollaborationError(error) ? error.status : 500;
