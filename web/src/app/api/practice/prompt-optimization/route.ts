...
type PracticePromptOptimizationBody = { requestId?: unknown; prompt?: unknown; mode?: unknown };
const modes = new Set(["image", "video", "audio"]);
...
export async function POST(request: Request) {
    ...
    try {
        ...
        return NextResponse.json({ code: status, data: null, msg: error instanceof Error ? error.message : "当前账号无权使用无限练习" }, { status });
    }
    ...
    let body: PracticePromptOptimizationBody;
    try {
    ...
    const mode = modes.has(String(body.mode || "")) ? (body.mode as CreativeGenerationMode) : "image";
