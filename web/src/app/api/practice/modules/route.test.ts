...
const mocks = vi.hoisted(() => ({ currentUser: vi.fn(), listCapabilities: vi.fn() }));
vi.mock("@/lib/auth/session", () => ({ getCurrentUser: mocks.currentUser }));
vi.mock("@/lib/server/practice-module-service", () => ({ listPracticeModuleCapabilities: mocks.listCapabilities }));
...
describe("/api/practice/modules", () => {
    beforeEach(() => {
        ...
        mocks.listCapabilities.mockResolvedValue([{ module: "script", mode: "manual", available: true, models: [], inputSchema: [], outputType: "text" }]);
    });
    ...
    it("returns the public capability envelope for a school member", async () => {
        ...
        expect(await response.json()).toEqual({ code: 200, data: { modules: expect.any(Array) }, msg: "ok" });
    });
    ...
    it("requires login and maps school access errors", async () => {
        ...
        mocks.listCapabilities.mockRejectedValueOnce(Object.assign(new Error("当前账号不是老师或学生"), { status: 403 }));
