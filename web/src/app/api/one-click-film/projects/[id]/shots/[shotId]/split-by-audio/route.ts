...
/**
 * 一键成片的按音频拆镜，语义与创作工坊完全一致（预览 + 追加式应用），
 * 只是项目归属校验换成 one-click-film 前缀，避免跨模块串项目。
 *
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; shotId: string }> }) {
    ...
    try {
        ...
        if (!project.sourceHandoffId?.startsWith("one-click-film:")) throw new DramaLabAudioSplitError("一键成片项目不存在", 404);
