...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; episodeId: string }> }) {
    ...
    try {
        ...
        const child = await startDramaLabWorkflow({
            ...
            options: { mode: "storyboard_extract", scope: "current", autoExport: false },
        ...
        return NextResponse.json({ code: 500, data: null, msg: error instanceof Error ? error.message : "分镜拆解失败" }, { status: 500 });
    }
}
