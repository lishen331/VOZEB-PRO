...
export async function PATCH(request: Request, context: Context) {
    ...
    try {
        ...
        await safeRecordAuditLog({ action: "admin.course.material.update", actor: auditActorFromRequest(request, user), target: { type: "course_material", id } });
        return schoolApiOk(result);
    } catch (error) {
        return schoolApiFailure(error, "更新课程资料失败");
    }
}
...
export async function DELETE(request: Request, context: Context) {
    ...
    try {
        ...
        await safeRecordAuditLog({ action: "admin.course.material.delete", actor: auditActorFromRequest(request, user), target: { type: "course_material", id } });
        return schoolApiOk(result);
    } catch (error) {
        return schoolApiFailure(error, "删除课程资料失败");
    }
}
