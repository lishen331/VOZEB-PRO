...
import { createPlatformChapter } from "@/lib/server/school-course-service";
...
export async function POST(request: Request, context: Context) {
    ...
    try {
        return schoolApiOk(await createPlatformChapter(user.id, (await context.params).id, { ...parsed.data, title: typeof parsed.data.title === "string" ? parsed.data.title : "" }));
    } catch (error) {
        return schoolApiFailure(error, "创建章节失败");
    }
}
