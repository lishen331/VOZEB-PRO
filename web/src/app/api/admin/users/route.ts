...
export async function POST(request: Request) {
    const currentUser = await getCurrentUser();
    if (!currentUser) return NextResponse.json({ error: "请先登录" }, { status: 401 });
    if (!hasAdminPermission(currentUser, "users.manage")) return NextResponse.json({ error: "当前管理员没有创建普通用户的职责权限" }, { status: 403 });
    ...
    try {
        ...
        if (body.role !== undefined || body.adminPermissions !== undefined) return NextResponse.json({ error: "用户运营不能设置管理员角色或职责权限" }, { status: 400 });
