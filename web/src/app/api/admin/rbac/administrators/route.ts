...
export async function GET() {
    ...
    try { return NextResponse.json({ items: await listPlatformAdministrators() }); } catch (error) { return failure(error, "读取管理员列表失败"); }
}
...
export async function POST(request: Request) {
    ...
    } catch (error) { return failure(error, "创建管理员失败"); }
}
