...
import { getAdminIp, updateAdminIp, type AdminIpPatchInput } from "@/lib/server/ip-library-admin-service";
...
export async function PATCH(request: Request, context: Context) {
    ...
    try {
        ...
        await safeRecordAuditLog({ action: "admin.ip.update", actor: auditActorFromRequest(request, user), target: { type: "ip", id }, metadata: { status: record.status, visibility: record.visibility, authorizationMode: record.authorizationMode } });
