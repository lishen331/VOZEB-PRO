...
describe("admin IP school grants route", () => {
    beforeEach(() => {
        ...
        mocks.create.mockResolvedValue({ id: "grant-a", ipId: "ip-a", schoolId: "school-a", mode: "exclusive", status: "active" });
    });

    it("requires education duty and audits identifiers without private note", async () => {
        expect((await GET(new Request("http://localhost/api/admin/ip-library/ip-a/schools"), context)).status).toBe(200);
        const response = await POST(jsonRequest({ schoolId: "school-a", mode: "exclusive", startsAt: "2026-08-19T00:00:00.000Z", note: "线下合同内容" }), context);
        expect(response.status).toBe(200);
        expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: "admin.ip.grant.create", target: { type: "ip_school_grant", id: "grant-a" }, metadata: { ipId: "ip-a", schoolId: "school-a", mode: "exclusive", status: "active" } }));
