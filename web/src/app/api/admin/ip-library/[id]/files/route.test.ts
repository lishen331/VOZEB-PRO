...
describe("admin IP content files route", () => {
    beforeEach(() => {
        ...
        mocks.list.mockResolvedValue([{ id: "file-one", ipId: "ip-one", subIpId: "sub-ip-one", kind: "text", status: "ready" }]);
        mocks.upload.mockResolvedValue({ id: "file-one", ipId: "ip-one", subIpId: "sub-ip-one", kind: "text", originalName: "story.txt", status: "ready", storageKey: "private/key" });
    });

    it("lists files and uploads multipart content for content managers", async () => {
        const listResponse = await GET(new Request("http://localhost/api/admin/ip-library/ip-one/files?subIpId=sub-ip-one"), context);
        expect(await listResponse.json()).toMatchObject({ code: 0, data: [{ id: "file-one" }] });
        expect(mocks.list).toHaveBeenCalledWith("admin-one", "ip-one", "sub-ip-one");

        const form = new FormData();
        form.set("subIpId", "sub-ip-one");
        form.set("kind", "text");
        ...
        expect(mocks.upload).toHaveBeenCalledWith("admin-one", "ip-one", "sub-ip-one", "text", expect.any(File));
        expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: "admin.ip.file.upload", target: { type: "ip_file", id: "file-one" }, metadata: { ipId: "ip-one", subIpId: "sub-ip-one", kind: "text", status: "ready" } }));
        expect(JSON.stringify(mocks.audit.mock.calls)).not.toContain("private/key");
    });

    it("requires a child IP before accepting a file", async () => {
        const form = new FormData();
        form.set("kind", "text");
        form.set("file", new File(["正文"], "story.txt", { type: "text/plain" }));

        expect((await POST(new Request("http://localhost/api/admin/ip-library/ip-one/files", { method: "POST", body: form }), context)).status).toBe(400);
