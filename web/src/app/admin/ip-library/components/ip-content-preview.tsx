...
export function IpContentPreview({ ipId, file, compact = false }: { ipId: string; file?: IpContentFileRecord; compact?: boolean }) {
    ...
    if (file.kind === "text") {
        return <pre className={`${compact ? "max-h-32" : "max-h-72"} overflow-auto whitespace-pre-wrap break-words rounded-md bg-zinc-50 p-3 text-xs leading-5 text-zinc-700 dark:bg-zinc-900 dark:text-zinc-200`}>{file.extractedText || "文本内容为空"}</pre>;
    }
