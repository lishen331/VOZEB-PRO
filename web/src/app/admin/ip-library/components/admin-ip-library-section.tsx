...
type IpForm = { title: string; slug: string; summary?: string; coverAssetId?: string; visibility: IpVisibility; authorizationMode: IpAuthorizationMode };
type VersionItemForm = { kind: IpAssetKind; category: IpItemCategory; title: string; summary?: string; textContent?: string; assetId?: string; sortOrder?: number };
type VersionForm = { title: string; summary?: string; items: VersionItemForm[] };
type GrantForm = { schoolId: string; startsAt: string; endsAt?: string; note?: string };
...
function IpContentPanel({ canManageContent, canManageEducation }: { canManageContent: boolean; canManageEducation: boolean }) {
    ...
    const setIpStatus = (ip: AdminIp, next: "published" | "disabled") =>
        modal.confirm({
            title: next === "published" ? `发布“${ip.title}”` : `停用“${ip.title}”`,
            content: next === "published" ? "发布后符合权限的学校成员可以查看和引用当前版本。" : "停用后禁止新的查看、引用和下载，历史版本仍保留。",
            okText: next === "published" ? "确认发布" : "确认停用",
            cancelText: "取消",
            async onOk() {
                await adminIpLibraryApi.update(ip.id, { status: next });
                message.success(next === "published" ? "IP 已发布" : "IP 已停用");
                await load();
            },
        });
    const actions = (ip: AdminIp) => (
        <Space size={0} wrap>
            ...
            {canManageContent && ip.status !== "disabled" ? (
                <Button
                    type="text"
                    size="small"
                    danger={ip.status === "published"}
                    icon={ip.status === "draft" ? <Send className="size-3.5" /> : <Ban className="size-3.5" />}
                    onClick={() => setIpStatus(ip, ip.status === "draft" ? "published" : "disabled")}
                >
                    {ip.status === "draft" ? "发布" : "停用"}
                </Button>
            ) : null}
        </Space>
    );
        ...
        },
        {
            ...
            render: (_, ip) => (
                <div className="space-y-1">
                    ...
                    <div className="text-xs text-zinc-500">{ip.visibility === "public" ? "公共 IP" : ipAuthorizationLabel(ip.authorizationMode)}</div>
                </div>
            ),
        },
        {
                ...
                {items.map((ip) => (
                    <article key={ip.id} className="rounded-md border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
                        ...
                        <div className="mt-2 text-xs text-zinc-500">
                            {ip.visibility === "public" ? "公共 IP" : ipAuthorizationLabel(ip.authorizationMode)} · v{ip.versionNumber || 0} · {ip.itemCount} 项
                        </div>
            ...
            >
                <Form form={form} layout="vertical" onFinish={save} className="pt-2">
                    ...
                    <div className="grid gap-x-3 sm:grid-cols-2">
                        ...
                        <Form.Item noStyle shouldUpdate={(before, after) => before.visibility !== after.visibility}>
                            {({ getFieldValue }) => (
                                <Form.Item name="authorizationMode" label="授权方式" rules={[{ required: true }]}>
                                    <Select disabled={getFieldValue("visibility") === "public"} options={authorizationOptions} />
                                </Form.Item>
                            )}
                        </Form.Item>
                    </div>
...
function VersionDrawer({ ip, canManage, onClose, onChanged }: { ip?: AdminIp; canManage: boolean; onClose: () => void; onChanged: () => Promise<void> }) {
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const result = await adminIpLibraryApi.listVersions(ip.id, { page, pageSize: PAGE_SIZE });
    ...
    useEffect(() => void load(), [load]);
    const create = async (values: VersionForm) => {
        if (!ip) return;
        setSaving(true);
        try {
            await adminIpLibraryApi.createVersion(ip.id, values);
    ...
    return (
            ...
            extra={
                canManage ? (
                    <Button
                        ...
                        onClick={() => {
                            form.resetFields();
                            form.setFieldsValue({ items: [{ kind: "text", category: "story_summary", title: "", textContent: "", sortOrder: 0 }] });
                            setCreating(true);
                        }}
                    >
                        新建版本
        ...
        >
            <div className="space-y-3" aria-busy={loading}>
                {versions.map((version) => (
                    <section key={version.id} className="rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
                        <div className="flex items-start justify-between gap-3">
                            ...
                            <Space>
                                ...
                                {canManage && version.status === "draft" ? (
                                    <Button size="small" type="primary" loading={saving} onClick={() => void publish(version)}>
                                        发布
                                    </Button>
                                ) : null}
                            </Space>
                        ...
                        <VersionItems items={version.items} />
                    </section>
                ))}
            </div>
            ...
            <Modal title="创建不可覆盖的新版本" open={creating} destroyOnHidden width="min(720px, 100vw)" okText="创建草稿" cancelText="取消" confirmLoading={saving} onCancel={() => setCreating(false)} onOk={() => form.submit()}>
                        ...
                        {(fields, { add, remove }) => (
                            <div className="space-y-3">
                                {fields.map((field, index) => (
                                    <VersionItemEditor key={field.key} field={field} index={index} remove={() => remove(field.name)} />
                                ))}
                                <Button block icon={<Plus className="size-4" />} onClick={() => add({ kind: "text", category: "story_summary", title: "", textContent: "", sortOrder: fields.length })}>
                                    添加内容项
                                </Button>
                            </div>
                        )}
...
function VersionItemEditor({ field, index, remove }: { field: { key: number; name: number }; index: number; remove: () => void }) {
    return (
        <div className="rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
            ...
            <div className="grid gap-x-3 sm:grid-cols-2">
                <Form.Item name={[field.name, "kind"]} label="类型" rules={[{ required: true }]}>
                    <Select options={kindOptions} onChange={() => undefined} />
                </Form.Item>
                <Form.Item noStyle shouldUpdate>
                    {({ getFieldValue, setFieldValue }) => {
                        ...
                        const selected = getFieldValue(["items", field.name, "category"]);
                        if (selected && !options.some((item) => item.value === selected)) queueMicrotask(() => setFieldValue(["items", field.name, "category"], options[0]?.value));
                        return (
            ...
            <Form.Item noStyle shouldUpdate>
                {({ getFieldValue }) => {
                    const kind = (getFieldValue(["items", field.name, "kind"]) || "text") as IpAssetKind;
                    return kind === "text" ? (
                        <Form.Item name={[field.name, "textContent"]} label="文本正文" rules={[{ required: true, message: "请填写文本正文" }]}>
                            <Input.TextArea rows={5} />
                        </Form.Item>
                    ) : (
                        <Form.Item name={[field.name, "assetId"]} label="媒体素材" rules={[{ required: true, message: "请选择媒体素材" }]}>
                            <AssetSelect kind={kind} placeholder={`分页搜索${kindLabel[kind]}素材`} />
                        </Form.Item>
                    );
                }}
            </Form.Item>
...
function VersionItems({ items }: { items: IpItemRecord[] }) {
                    ...
                    <article key={ip.id} className="flex items-center justify-between gap-3 rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-zinc-500">
                                {ipAuthorizationLabel(ip.authorizationMode)} · v{ip.versionNumber}
                            </p>
                        </div>
...
function GrantDrawer({ ip, onClose }: { ip?: AdminIp; onClose: () => void }) {
    ...
    const create = async (values: GrantForm) => {
        ...
        setLoading(true);
        try {
            await adminIpLibraryApi.createGrant(ip.id, { ...values, mode: ip.authorizationMode, startsAt: localToIso(values.startsAt), endsAt: values.endsAt ? localToIso(values.endsAt) : undefined });
        ...
        <Drawer
            ...
            extra={
                <Button
                    ...
                    onClick={() => {
                        form.resetFields();
                        form.setFieldsValue({ startsAt: toLocalInput(new Date().toISOString()) });
                        setCreating(true);
                        void searchSchools();
                    }}
                >
                    ...
                    <article key={grant.id} className="rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <div className="mt-1 text-xs text-zinc-500">
                                    {formatTime(grant.startsAt)} 至 {grant.endsAt ? formatTime(grant.endsAt) : "长期"}
                                </div>
                            </div>
...
function AssetSelect({ kind, placeholder, value, onChange, onAssetChange }: { kind: Asset["kind"]; placeholder: string; value?: string; onChange?: (value: string | undefined) => void; onAssetChange?: (value: string | undefined) => void }) {
    ...
    const search = async (keyword = "") => {
        ...
        setLoading(true);
        try {
            const result = await listLibraryAssetPage({ page: 1, pageSize: PAGE_SIZE, kind, keyword: keyword.trim() || undefined });
            if (id === requestId.current) setAssets(result.assets);
        } finally {
            if (id === requestId.current) setLoading(false);
        }
    };
