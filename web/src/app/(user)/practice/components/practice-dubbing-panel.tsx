...
export default function PracticeDubbingPanel({ capability, onCreated }: PracticePanelProps) {
    ...
    const submit = async () => {
        ...
        setBusy(true);
        try {
            onCreated((await practiceApi.createSession({ module: "dubbing", mode: "workflow", title: "配音练习", input: { text: text.trim(), ...workflowInput }, logicalModelId: model, clientRequestId: crypto.randomUUID() })).session);
        } finally {
            setBusy(false);
        }
    };
