...
export default function PracticeMusicPanel({ capability, onCreated }: PracticePanelProps) {
    ...
    const submit = async () => {
        ...
        setBusy(true);
        try {
            onCreated((await practiceApi.createSession({ module: "music", mode: "workflow", title: "音乐练习", input: { prompt: prompt.trim(), ...workflowInput }, logicalModelId: model, clientRequestId: crypto.randomUUID() })).session);
        } finally {
            setBusy(false);
        }
    };
    return (
        <div className="space-y-4">
            ...
            <label className="block text-sm font-medium">
                音乐需求
                <Input.TextArea value={prompt} onChange={(event) => setPrompt(event.target.value)} autoSize={{ minRows: 7, maxRows: 14 }} className="!mt-2" placeholder="描述情绪、节奏、乐器和使用场景" />
            </label>
            ...
            <Button type="primary" icon={<Music2 className="size-4" />} loading={busy} disabled={!capability.available || !model || !prompt.trim()} onClick={() => void submit()}>
                生成音乐
            </Button>
        </div>
    );
