...
export function PracticeSessionResult({ module, session, onRetry, onRefresh }: { module: PracticeModuleKind; session?: PracticeSession | null; onRetry: () => void; onRefresh: () => void }) {
    ...
    if (session.status === "queued" || session.status === "running" || session.result?.status === "pending" || session.result?.status === "running")
        return <p className="mt-4 flex items-center gap-2 text-sm text-muted-foreground"><Spin size="small" />{session.status === "queued" ? "已排队，等待生成" : "正在生成，请刷新查看结果"}<Button type="text" size="small" icon={<RefreshCw className="size-3.5" />} onClick={onRefresh} aria-label="刷新练习状态" /></p>;
    if (session.status === "failed" || session.result?.status === "error")
        return <div className="mt-4 flex items-center justify-between gap-3 text-sm text-red-600 dark:text-red-300"><span>{session.errorMessage || session.result?.error || "练习失败，请重试"}</span><Button size="small" onClick={onRetry}>重试</Button></div>;
