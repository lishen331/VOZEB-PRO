...
export function WorkflowOptionalFields({ capability, value, onChange }: { capability: PracticeModuleCapability; value: Record<string, unknown>; onChange: (key: string, next: unknown) => void }) {
    ...
    return <div className="grid gap-3 sm:grid-cols-2">{fields.map((field) => <WorkflowField key={field.key} field={field} value={value[field.key]} onChange={(next) => onChange(field.key, next)} />)}</div>;
}

function WorkflowField({ field, value, onChange }: { field: PracticeModuleInputField; value: unknown; onChange: (value: unknown) => void }) {
    if (field.type === "number") return <label className="block text-sm font-medium">{field.label}<InputNumber value={typeof value === "number" ? value : undefined} onChange={(next) => onChange(next ?? undefined)} className="!mt-2 !w-full" /> </label>;
    if (field.type === "enum") return <label className="block text-sm font-medium">{field.label}<Select value={typeof value === "string" ? value : undefined} onChange={onChange} options={(field.options || []).map((option) => ({ value: option, label: option }))} className="!mt-2 !w-full" /> </label>;
    return <label className="flex items-center justify-between gap-3 border border-border p-3 text-sm font-medium">{field.label}<Switch checked={value === true} onChange={onChange} /></label>;
}
