...
export default function PracticeCharacterPanel({ capability, onCreated }: PracticePanelProps) {
    ...
    const submit = async () => {
        ...
        setBusy(true);
        try {
            onCreated((await practiceApi.createSession({
                module: "character", mode: "workflow", title: "角色练习", workflowCode,
                ...
                logicalModelId: model, clientRequestId: crypto.randomUUID(),
            })).session);
        } finally { setBusy(false); }
    };
    return <div className="space-y-4">
        <ModelField capability={capability} value={model} onChange={setModel} />
        <label className="block text-sm font-medium">角色视图<Segmented value={view} onChange={(value) => setView(value as "main" | "multi")} options={[{ label: "主形象", value: "main" }, { label: "多视图", value: "multi", disabled: !canMultiView }]} className="!mt-2" /></label>
        <label className="block text-sm font-medium">主形象参考图{view === "multi" ? "（必需）" : "（可选）"}<input type="file" accept="image/*" disabled={uploading} onChange={(event) => void chooseImage(event.target.files?.[0])} className="mt-2 block w-full text-sm" />{image ? <img src={image.url} alt="已选择的角色参考图" className="mt-2 max-h-48 w-full object-contain" /> : null}</label>
        <label className="block text-sm font-medium">角色描述<Input.TextArea value={prompt} onChange={(event) => setPrompt(event.target.value)} autoSize={{ minRows: 5, maxRows: 10 }} className="!mt-2" placeholder="描述外观、服装、气质和视觉风格" /></label>
        ...
        <Button type="primary" icon={<ImagePlus className="size-4" />} loading={busy || uploading} disabled={!capability.available || !model || !prompt.trim() || (view === "multi" && !image?.storageKey)} onClick={() => void submit()}>生成角色图</Button>
    </div>;
}
