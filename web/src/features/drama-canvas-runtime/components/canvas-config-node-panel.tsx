...
function CanvasImageReferenceRolesPopover({ references, roles, theme, onChange }: { references: CanvasResourceReference[]; roles?: CanvasNodeMetadata["imageReferenceRoles"]; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onChange: (roles: NonNullable<CanvasNodeMetadata["imageReferenceRoles"]>) => void }) {
    ...
    const current = roles || {};
    return (
        <Dropdown trigger={["click"]} placement="top" popupRender={() => (
            <div className="w-64 rounded-xl border p-2 shadow-xl" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                {references.length ? references.map((reference) => <div key={reference.nodeId} className="flex items-center gap-2 rounded-lg px-2 py-1.5"><span className="min-w-0 flex-1 truncate text-xs">{reference.label} · {reference.title}</span><select className="h-7 max-w-28 rounded-md border bg-transparent px-1 text-[11px]" value={current[reference.nodeId] || "original"} onChange={(event) => onChange({ ...current, [reference.nodeId]: event.target.value as keyof typeof labels })}>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>) : <div className="px-2 py-2 text-[11px] opacity-60">暂无已连接图片</div>}
            </div>
        )}>
            <Button type="text" data-canvas-no-drag className="canvas-compact-control !h-9 !w-full !justify-start !rounded-none !border-0 !bg-transparent !px-2 !shadow-none" style={{ color: theme.node.muted }} aria-label="设置参考图用途"><Tag className="mr-1.5 size-3.5" /><span className="truncate text-[11px]">参考图用途</span></Button>
        </Dropdown>
    );
}
