        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                                ...
                                : node.id === nodeId && isConfigNode
                                  ? {
                                        ...
                                        metadata: {
                                            ...
                                            errorDetails: hasSuccess || hasReview ? undefined : hasDeferred ? "图片仍在后台生成，系统会继续查询原任�? : "全部图片生成失败",
                                        },
                                    }
                                  : node.id === nodeId && isEmptyImageNode
                                    ? {
                                          ...
                                          metadata: {
                                              ...
                                              errorDetails: hasSuccess ? undefined : node.metadata?.errorDetails || (hasDeferred ? "图片仍在后台生成，系统会继续查询原任�? : "全部图片生成失败"),
                                          },
                                      }
                                    : node.id === rootId && !hasSuccess
                                      ? {
                                            ...
                                            metadata: {
                                                ...
                                                errorDetails: hasReview ? undefined : hasDeferred ? "图片仍在后台生成，系统会继续查询原任�? : "全部图片生成失败",
                                            },
                                        }
                                      : node,
                        ),
                ...
                if (videoTaskId && videoFailure && videoFailure !== "upstream_failed") {
                    message.info("视频仍在后台生成，系统会继续查询原任�?);
    ...
    const recoverReviewedNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                if (isGenerationCanceled(error)) return;
                const errorDetails = error instanceof Error ? error.message : "重新检查任务失�?;
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            if (useReferenceImages && !retryReferenceImages) {
                message.error("参考图片已丢失，无法继续重�?);
                setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails: "参考图片已丢失，无法继续重�? } } : item)));
                return;
            }
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    if (!context) throw new Error("视频生成上下文已丢失，无法继续重�?);
