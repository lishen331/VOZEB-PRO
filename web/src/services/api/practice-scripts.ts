...
export const practiceScriptsApi = {
    ...
    create(input: { title: string; sourceType?: string; idea?: string; mode?: "short_story" | "long_novel"; carrierType?: string; projectParameters?: Record<string, unknown> }) {
    ...
    confirmArtifact(id: string, artifactId: string, stageKey: string, runId?: string, chatSessionId?: string) {
        return request<Record<string, unknown>>(`/api/practice/scripts/${encodeURIComponent(id)}/confirmations`, json("POST", { artifactId, stageKey, runId, chatSessionId }));
    },
