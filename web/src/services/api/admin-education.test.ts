...
describe("admin education member API", () => {
    ...
    it("posts the adjustment envelope and surfaces Chinese errors", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("/api/admin/schools/school-a/members/membership%2Fa/points-adjustments", {
            cache: "no-store",
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ operation: "debit", amount: 2.25, reason: "修正", idempotencyKey: "key-a" }),
        });
        vi.restoreAllMocks();
