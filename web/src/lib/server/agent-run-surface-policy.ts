...
export function agentPlannerSystemPrompt(surface: CreativeSurface, fallbackExample: string, siteTitle: string) {
    ...
    const surfaceRules =
        surface === "canvas"
            ? "要求整理、排版、优化画布节点布局时 intent=canvas_operation，canvasOperation={type:layout,scope:all或selected}，deliverables=[]，不生成说明文字节点、不改正文、不宣称已经执行成功。使用canvasSnapshot.layout的完整几何信息；用户明确说全部/全画布时scope=all，明确说选中时scope=selected，未说明范围时有选中节点用selected否则all。解释如何整理但未要求执行属于conversation。删除节点用canvas_operation与canvasOperation={type:delete_nodes,ids:[真实节点ID]}；断开连线用type:disconnect和真实连线ID。只能提出待确认方案，不能宣称已删除；有选中节点时不得越过该范围。ids必须明确列出，禁止通配符，禁止将整理理解成删除。普通画布尚无通用分组容器，分组请求在reply说明未支持，不伪造分组成功。创建节点或生成媒体产物时为 generation。用户要求修改已有画布产物时必须填写该节点真实 targetNodeId。选中文本/提示词节点并要求修改、优化或改写时，只规划一个 type=text 的原位编辑任务，targetNodeId 必须是该文本节点；除非用户同时明确要求生成媒体，否则禁止规划图片、视频或音频任务。canvasSnapshot.selectedNodeIds 是用户本轮明确选中并展示在输入框中的附件：非空时，当前编辑任务必须优先且只能从这些节点选择 targetNodeId，禁止被 conversationContext 的上一张、旧主体或其他未选中画布节点覆盖；只有本轮没有选中节点时，才允许结合会话记忆选择旧节点。"
