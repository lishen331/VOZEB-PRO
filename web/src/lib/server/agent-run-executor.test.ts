...
vi.mock("@/lib/server/agent-run-store", async (importOriginal) => {
    ...
    return { ...actual, getAgentRun: vi.fn(async () => mocks.run), updateAgentRunById: mocks.updateAgentRunById, updateAgentRunTaskById: mocks.updateAgentRunTaskById };
});
...
describe("executeAgentRun backend settings", () => {
    ...
    it("blocks a reference-required skill when the planner omits actual task references", async () => {
        ...
        mocks.run.plannerAudit = {
            schemaVersion: 2,
            mode: "model",
            skills: [{ id: "retouch", name: "retouch", description: "", plannerSummary: "", instructions: "", enabled: true, keywords: [], workspaces: ["image"], action: "edit", requiresReference: true, defaultConfig: {} }],
        };
    ...
    it("uses a stable billing identity within a planning attempt and a new one on explicit retry", async () => {
        const keys: string[] = [];
        mocks.getAuthSettings.mockResolvedValue(settings("image-model", "image-channel"));
        mocks.fetchInternalApi.mockImplementation(async (_url, init) => {
            keys.push(new Headers(init?.headers).get("x-vozeb-pro-points-idempotency-key") || "");
            return Response.json({ choices: [{ message: { content: JSON.stringify(conversationPlan("planner", "hello")) } }] });
        });
        for (const planningAttempt of [0, 0, 1]) {
            mocks.run = runFixture({ surface: "chat", planningAttempt, prompt: "hello" });
            await executeAgentRun(mocks.run, "http://localhost", "session=test");
        }
    ...
    it("does not pause healthy siblings when another model needs review", async () => {
        mocks.run = runWithTasks([imageTask("unknown"), imageTask("healthy")]);
        mocks.getAuthSettings.mockResolvedValue(settings("image-model", "image-channel"));
        let created = 0;
        let polls = 0;
        mocks.fetchInternalApi.mockImplementation(async (url: string, init?: RequestInit) => {
            if (init?.method === "POST" && url.endsWith("/api/image-tasks")) return Response.json({ task: { id: ++created === 1 ? "unknown-child" : "healthy-child" } });
            if (url.endsWith("/unknown-child")) return Response.json({ task: { status: "running", needsReview: true } });
            if (url.endsWith("/healthy-child")) return Response.json({ task: ++polls === 1 ? { status: "running" } : { status: "success", result: { url: "https://cdn.example.com/ok.png" } } });
            throw new Error("unexpected request");
        });
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        expect(mocks.run?.status).toBe("running");
        await executeAgentRun(mocks.run!, "http://localhost", "session=test");
        expect(mocks.run?.tasks.find((t) => t.id === "healthy")?.status).toBe("completed");
        expect(mocks.run?.status).toBe("paused");
        expect(created).toBe(2);
        expect(mocks.fetchInternalApi.mock.calls.filter(([url]) => String(url).endsWith("/unknown-child"))).toHaveLength(1);
    });
    ...
    it("preserves generated media dimensions in canvas output ops", () => {
        const task = { ...imageTask("image-one"), attempts: 1, result: { url: "https://cdn.example.com/output.png", width: 1024, height: 1024, mimeType: "image/png" } } as AgentRunTask;
        const output = taskResultOps("agent-run", 0, task);
        expect(output.ops[0]).toMatchObject({ type: "update_node", id: "output-agent-run-0-0", metadata: { remoteUrl: "https://cdn.example.com/output.png", naturalWidth: 1024, naturalHeight: 1024, mimeType: "image/png", size: task.ratio } });
    ...
    it("runs an explicitly selected generation model without a default text model", async () => {
        ...
        mocks.getCreativeConversationContext.mockResolvedValue({ summary: "同一商品使用红色包装", summaryThroughSequence: 1, recentMessages: [] });
        const manualSettings = settings("image-model", "image-channel") as unknown as { defaultModels: { textModel: string }; systemChannels: Array<{ id: string }>; logicalModels: Array<{ capability: string }> };
    ...
    it("keeps project authorization context on a later text task after an earlier child was submitted", async () => {
        ...
        expect(JSON.parse(String(createCall?.[1]?.body))).toMatchObject({
            context: { conversationId: "conversation", runId: "agent-run", surface: "canvas", projectId: "project", billingContext, parentTaskId: "text-later", attemptNo: 1, clientRequestId: "request:text-later:1:1" },
        });
    ...
    it("pauses on needs_review and resumes the same child without another upstream creation", async () => {
        ...
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        expect(mocks.fetchInternalApi.mock.calls.filter((call) => call[1]?.method === "POST")).toHaveLength(1);
        expect(mocks.run).toMatchObject({ status: "paused", tasks: [expect.objectContaining({ status: "needs_review", taskId: "child-review", childTasks: [expect.objectContaining({ id: "child-review", status: "needs_review" })] })] });
        ...
        mocks.run = { ...mocks.run!, status: "running", tasks: mocks.run!.tasks.map((task) => ({ ...task, status: task.status === "needs_review" ? ("running" as const) : task.status })) };
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        ...
        expect(mocks.fetchInternalApi.mock.calls.filter((call) => call[1]?.method === "POST" && String(call[0]).endsWith("/api/image-tasks/child-review"))).toHaveLength(0);
        expect(polls).toBe(2);
        expect(mocks.run?.status).toBe("completed");
    });
    ...
    it("turns explicit canvas text-node content into a node result without calling the text task API", async () => {
        ...
        expect(completed?.data?.ops).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ type: "add_node", id: "output-agent-run-0-0", nodeType: "text", position: { x: 800, y: 96 }, metadata: expect.objectContaining({ content: "欢迎使用 VOZEB PRO Agent" }) }),
    ...
    it("accepts a strict JSON canvas plan and executes the model selected by the Agent", async () => {
        ...
        nextSettings.agentSkills = [
            { id: "skill-one", name: "商品视觉", description: "商品视觉规划", instructions: "保持商品一致", enabled: true, keywords: ["商品"], workspaces: ["canvas"], sourceVersion: "1.2.0", sourceCommit: "abcdef", sourceContentHash: "hash" },
        ];
    ...
    it("passes the persistent summary and recent messages to the planner", async () => {
        ...
        expect(JSON.parse(body.messages[1].content)).toMatchObject({ conversationContext: { summary: "用户正在制作统一的新中式女主角色。", recentMessages: [{ role: "assistant", content: "第二张采用红色服装。", sequence: 9 }] } });
        expect(mocks.getCreativeConversationContext).toHaveBeenCalledWith("conversation", "user", "agent-run");
    });
    it("lets the text model select a same-conversation media candidate for continuous creation", async () => {
        ...
        mocks.getAuthSettings.mockResolvedValue(canvasSettings("image-default", "image-default-channel"));
        const plan = { ...canvasPlan("image-default"), deliverables: [{ ...canvasPlan("image-default").deliverables[0], assetIds: [memoryAsset.id] }] };
        mocks.fetchInternalApi.mockImplementation(async (url: string, init?: RequestInit) => {
        ...
        expect(JSON.parse(planningBody.messages[1].content)).toMatchObject({ referenceContext: { source: "conversation-memory-candidates" }, referencedAssets: [{ id: "asset-memory", title: "上一张角色图" }] });
    ...
    it("answers ordinary conversation without creating canvas ops or media tasks", async () => {
        ...
        mocks.fetchInternalApi.mockResolvedValue(
            Response.json({
                output: [
                    { type: "function_call", name: "create_agent_plan", arguments: JSON.stringify({ ...canvasPlan("image-default"), intent: "conversation", reply: "在的，你可以直接和我聊天，也可以让我操作当前画布。", decisions: [], deliverables: [] }) },
                ],
            }),
        );
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
    ...
    it("plans chat media without canvas ops, links the child task and registers a stable asset", async () => {
        ...
        mocks.getCreativeAssetsByIds.mockResolvedValue([
            { id: "asset-source", userId: "user", conversationId: "conversation", ordinal: 0, type: "image", status: "ready", title: "参考角色", remoteUrl: "https://cdn.example.com/source.png", metadata: {}, createdAt: 1, updatedAt: 1 },
            { id: "asset-style", userId: "user", conversationId: "conversation", ordinal: 1, type: "image", status: "ready", title: "风格参考", remoteUrl: "https://cdn.example.com/style.png", metadata: {}, createdAt: 1, updatedAt: 1 },
        ]);
        mocks.getAuthSettings.mockResolvedValue(canvasSettings("image-default", "image-default-channel"));
        ...
        expect(mocks.linkStoredGenerationTask).toHaveBeenCalledWith("image", "child-chat", { conversationId: "conversation", runId: "agent-run", surface: "chat", projectId: undefined, parentTaskId: "agent-run", attemptNo: 1 });
    ...
    it("uses completed dependency assets as real references for downstream video", async () => {
        ...
        const nextSettings = settings("image-model", "image-channel") as unknown as { defaultModels: { videoModel: string }; systemChannels: Array<Record<string, unknown>>; logicalModels: Array<Record<string, unknown>> };
    ...
    it("passes explicit video flags and audio speed to child task routes", async () => {
        ...
        const nextSettings = settings("image-model", "image-channel") as unknown as { defaultModels: { videoModel: string; audioModel: string }; systemChannels: Array<Record<string, unknown>>; logicalModels: Array<Record<string, unknown>> };
        nextSettings.defaultModels.videoModel = "video-model";
    ...
    it("passes drama project context to planning without creating canvas operations", async () => {
        ...
        mocks.getAuthSettings.mockResolvedValue(canvasSettings("image-default", "image-default-channel"));
        mocks.fetchInternalApi.mockResolvedValue(
            Response.json({ output: [{ type: "function_call", name: "create_agent_plan", arguments: JSON.stringify({ ...canvasPlan("image-default"), intent: "conversation", reply: "因为当前冲突迫使角色主动离开。", decisions: [], deliverables: [] }) }] }),
        );
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        const planningBody = JSON.parse(String(mocks.fetchInternalApi.mock.calls[0][1]?.body)) as { messages: Array<{ content: string }> };
    ...
    it("emits an idempotent project handoff for chat without creating media tasks", async () => {
        ...
        const sourceAsset = { id: "asset-source", userId: "user", conversationId: "conversation", ordinal: 0, type: "image", status: "ready", title: "女主设定", remoteUrl: "https://cdn.example.com/hero.png", metadata: {}, createdAt: 1, updatedAt: 1 };
        mocks.getCreativeAssetsByIds.mockResolvedValue([sourceAsset]);
        mocks.getAuthSettings.mockResolvedValue(canvasSettings("image-default", "image-default-channel"));
        const plan = { ...canvasPlan("image-default"), deliverables: [], projectHandoff: { surface: "drama", title: "都市悬疑", summary: "女主追查失踪案", style: "写实电影感", ratio: "9:16", assetIds: ["asset-source"] } };
        ...
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        expect(mocks.fetchInternalApi.mock.calls.some(([url]) => /\/api\/(?:image|video|audio|text)-tasks/.test(String(url)))).toBe(false);
        expect(mocks.events.find((event) => event.type === "project.handoff")?.data).toMatchObject({ id: "handoff-agent-run", surface: "drama", title: "都市悬疑", assetIds: ["asset-source"], assets: [expect.objectContaining({ id: "asset-source" })] });
    ...
    it("ignores an invalid project handoff attached to an ordinary image plan", async () => {
        ...
        mocks.getAuthSettings.mockResolvedValue(canvasSettings("image-default", "image-default-channel"));
        const plan = { ...canvasPlan("image-default"), projectHandoff: { surface: "canvas", title: "", ratio: "1:1", assetIds: [""] } };
        mocks.fetchInternalApi.mockImplementation(async (url: string, init?: RequestInit) => {
            if (url.endsWith("/chat/completions")) return Response.json({ output: [{ type: "function_call", name: "create_agent_plan", arguments: JSON.stringify(plan) }] });
            if (init?.method === "POST" && url.endsWith("/api/image-tasks")) return Response.json({ task: { id: "child-image" } });
...
describe("partial success handling", () => {
    ...
    it("should set status to partial_success when some tasks succeed and some fail", async () => {
        ...
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        expect(mocks.run?.status).toBe("partial_success");
        expect(mocks.events).toContainEqual(expect.objectContaining({ type: "run.partial_success", data: expect.objectContaining({ completed: 1, failed: 1 }) }));
    });
    it("should include success and failure counts in partial_success event", async () => {
        ...
        expect(partialEvent?.data).toMatchObject({ completed: 2, failed: 3, assetIds: ["asset-1", "asset-2"] });
    ...
    it("should still mark as completed when all tasks succeed", async () => {
        ...
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        expect(mocks.run?.status).toBe("completed");
        expect(mocks.events).toContainEqual(expect.objectContaining({ type: "run.completed" }));
    });
    it("should mark as failed when all tasks fail and no assets generated", async () => {
        ...
        await executeAgentRun(mocks.run, "http://localhost", "session=test");
        expect(mocks.run?.status).toBe("failed");
        expect(mocks.events).toContainEqual(expect.objectContaining({ type: "run.failed" }));
    });
});
