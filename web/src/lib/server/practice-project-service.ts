...
export type PracticeProjectIdentity = { executionProfile: "open-source-practice"; schoolId: string; practiceSource: PracticeSource };
...
export async function createPracticeProject(actor: PracticeActor, input: PracticeProjectInput) {
    ...
    const identity: PracticeProjectIdentity = { executionProfile: "open-source-practice", schoolId: access.schoolId, practiceSource: input.source || { type: "blank" } };
...
export async function listPracticeProjects(actor: PracticeActor, input: { kind: PracticeProjectKind; page?: unknown; pageSize?: unknown }) {
    ...
    if (input.kind === "drama") return { kind: "drama" as const, ...(await listDramaProjectSummariesForUser(actor.id, { page, pageSize, executionProfile: "open-source-practice", schoolId: access.schoolId })) };
    return { kind: "canvas" as const, ...(await listCanvasProjectsForUser(actor.id, { page, pageSize, executionProfile: "open-source-practice", schoolId: access.schoolId })) };
}
