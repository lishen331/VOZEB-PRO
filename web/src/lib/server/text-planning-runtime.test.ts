...
describe("text planning runtime protocol matrix", () => {
    ...
    it("需要结构化结果时优先使用 Chat 原生函数工具", async () => {
        mockedFetch.mockResolvedValue(Response.json({ choices: [{ message: { tool_calls: [{ type: "function", function: { name: "make_plan", arguments: '{"result":"ok"}' } }] } }] }));
        ...
        expect(requestBody()).toMatchObject({ tools: [{ type: "function", function: tool }], tool_choice: { type: "function", function: { name: "make_plan" } } });
    });
    ...
    it("为 Responses 和 Gemini 发送各自的原生函数声明", async () => {
        ...
        expect(requestBody()).toMatchObject({ tools: [{ functionDeclarations: [tool] }], toolConfig: { functionCallingConfig: { mode: "ANY", allowedFunctionNames: ["make_plan"] } } });
    });
    ...
    it("接受自定义结果字段直接返回的结构化对象", async () => {
        ...
        const result = await requestStructuredText(requestInput(candidate("custom", { createPath: "/planner/run", requestTemplate: '{"prompt":"{{prompt}}"}', resultField: "data.plan" })));
    ...
    it("模型级 Responses 预设覆盖 New API 渠道的默认 Chat", async () => {
        const configured = candidate("newapi", { modelConfigs: { "model-one": { capability: "text", protocol: "compatible", createPath: "/responses" } } });
        mockedFetch.mockResolvedValue(Response.json({ output_text: "{}" }));
    ...
    it("模型级自定义协议不会因路径名为 responses 被误判", async () => {
        const configured = candidate("compatible", { modelConfigs: { "model-one": { capability: "text", protocol: "custom", createPath: "/responses", requestTemplate: '{"deployment":"{{model}}","prompt":"{{prompt}}"}', resultField: "payload.plan" } } });
    ...
    it("Gemini 原生预设使用 generateContent 并解析候选文本", async () => {
        mockedFetch.mockResolvedValue(Response.json({ candidates: [{ content: { parts: [{ text: "{}" }] } }] }));
        const result = await requestStructuredText(requestInput(candidate("compatible", { apiFormat: "gemini", createPath: "/models/:model:generateContent" })));
    ...
    it("自定义文本协议使用管理员模板、路径和结果字段", async () => {
        ...
        const result = await requestStructuredText(requestInput(candidate("custom", { createPath: "/planner/run", requestTemplate: '{"deployment":"{{model}}","conversation":"{{messages}}"}', resultField: "data.plan" })));
    ...
    it("自定义模板可以接收结构化 prompt_json 占位符", async () => {
        mockedFetch.mockResolvedValue(Response.json({ data: { plan: "{}" } }));
        await requestStructuredText(requestInput(candidate("custom", { createPath: "/planner/run", requestTemplate: '{"prompt_json":"{{prompt_json}}"}', resultField: "data.plan" })));
        expect(requestBody()).toMatchObject({ prompt_json: "test" });
    });
    it("只为上游协议作用域追加后缀，不改写服务端计费身份", async () => {
        mockedFetch.mockResolvedValue(chatJsonResponse());
        await requestStructuredText({ ...requestInput(candidate("newapi")), headers: { "x-vozeb-pro-points-idempotency-key": "planning-one", "idempotency-key": "planning-one" } });
        const headers = new Headers(mockedFetch.mock.calls[0]?.[1]?.headers);
    ...
    it("同一渠道不同模型的协议预设互不污染", async () => {
        const channel = candidate("compatible", {
            modelConfigs: { "model-one": { capability: "text", protocol: "compatible", createPath: "/responses" }, "model-two": { capability: "text", protocol: "compatible", createPath: "/chat/completions" } },
        }).channel;
    ...
    it("模型级流式配置覆盖渠道级设置", async () => {
        ...
        await expect(
            requestStructuredText({ ...requestInput(candidate("newapi", { streaming: { enabled: false }, modelConfigs: { "model-one": { capability: "text", streaming: { enabled: true, path: "/chat/stream", format: "sse" } } } })), stream: true }),
...
function requestInput(configured: TextPlanningCandidate) {
    return { origin: "http://127.0.0.1:3000", cookie: "session=test", candidate: configured, messages: [{ role: "user", content: "test" }], tool };
}
function candidate(protocol: NonNullable<SystemChannelAdvancedConfig>["protocol"], options: Partial<SystemChannelAdvancedConfig> & { id?: string; apiFormat?: "openai" | "gemini" } = {}): TextPlanningCandidate {
    ...
    const channel = { id, name: id, baseUrl: "https://example.com/v1", apiKey: "secret", apiFormat: options.apiFormat || "openai", models: ["model-one", "model-two"], enabled: true, advancedConfig } satisfies SystemModelChannel;
