...
export async function refundVideoTask(task: VideoTask) {
    ...
    if (!generationTaskShouldConsumePoints(task.executionProfile) || (task.status !== "error" && task.status !== "cancelled") || upstream.pointsCost === undefined || !upstream.pointsRecordId || upstream.refunded) return task;
