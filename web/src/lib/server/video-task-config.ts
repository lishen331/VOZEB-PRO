...
export function withVideoReferenceFidelity(prompt: string, references: readonly VideoGenerationReference[]) {
    ...
    return `${source}\n\n参考素材一致性要求：将${sourceLabel}作为主体身份、外观和场景的参考依据，禁止替换主体或生成无关场景。普通参考素材不等于首帧，不锁定参考图构图或镜头视角；严格执行用户文字中的剧情、动作顺序与运镜。参考图包含故事板或多宫格时，按用户要求理解各格内容并组织连续镜头，不把整张拼图当作固定开场画面。`;
}
