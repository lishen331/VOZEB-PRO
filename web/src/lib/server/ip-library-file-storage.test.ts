...
describe("IP library source file storage", () => {
    ...
    it("stores strict UTF-8 text and exposes extracted text", async () => {
        const record = await writeIpContentFile({ ipId: "ip-one", subIpId: "sub-one", fileId: "file-text", kind: "text", originalName: "设定.md", bytes: Buffer.from("# 世界观\n星海", "utf8"), uploadedByUserId: "admin-one" });
        ...
        await expect(writeIpContentFile({ ipId: "ip-one", subIpId: "sub-one", fileId: "bad-text", kind: "text", originalName: "坏编码.txt", bytes: Buffer.from([0xc3, 0x28]), uploadedByUserId: "admin-one" })).rejects.toMatchObject({ status: 415 });
    });
    ...
    it("detects real image bytes, rejects extension mismatches and extracts dimensions", async () => {
        const png = await sharp({ create: { width: 37, height: 23, channels: 3, background: "#336699" } })
            .png()
            .toBuffer();
        const record = await writeIpContentFile({ ipId: "ip-one", subIpId: "sub-one", fileId: "file-image", kind: "image", originalName: "角色.png", bytes: png, uploadedByUserId: "admin-one" });
        ...
        await expect(writeIpContentFile({ ipId: "ip-one", subIpId: "sub-one", fileId: "fake-image", kind: "image", originalName: "伪装.png", bytes: Buffer.from("not an image"), uploadedByUserId: "admin-one" })).rejects.toMatchObject({ status: 415 });
        await expect(writeIpContentFile({ ipId: "ip-one", subIpId: "sub-one", fileId: "wrong-extension", kind: "image", originalName: "角色.jpg", bytes: png, uploadedByUserId: "admin-one" })).rejects.toMatchObject({ status: 415 });
    });

    it("serves and removes local files only through the independent record", async () => {
        const record = await writeIpContentFile({ ipId: "ip-one", subIpId: "sub-one", fileId: "file-text", kind: "text", originalName: "story.txt", bytes: Buffer.from("正文"), uploadedByUserId: "admin-one" });
