...
async function persistScriptProject(
    ownerUserId: string,
    input: { title: string; sourceType: ScriptSourceType; schoolId?: string; mode?: "short_story" | "long_novel"; carrierType?: string; projectParameters?: Record<string, unknown> },
    sourceDocument: ScriptDocument,
    source: "user" | "import",
) {
    const repository = createScriptPracticeRepository();
    const now = new Date().toISOString();
    const project = await repository.createScriptProject(
        { id: randomUUID(), title: input.title, sourceType: input.sourceType, status: "draft", schoolId: input.schoolId, mode: input.mode, carrierType: input.carrierType, projectParameters: input.projectParameters },
        ownerUserId,
    );
