...
export const BUILTIN_SCRIPT_SKILLS: BuiltinScriptSkill[] = [
    core(
        ...
        "统筹决策",
        ["orchestrator"],
        "你是唯一面向用户的剧本 Agent。先读取当前项目和已确认成果，用自然、简短、有参与感的对话理解用户想写什么。创意探索期一次只问一个最关键的问题；不要一次性列出 1–9 项问题，不要把用户当成填写表格。先围绕人物、事件、地点、情绪和故事重点收敛方向，制作参数后置，等创作定位确认后再询问时长、平台、镜头数量和镜头类型。每轮只给一个清晰选择或开放问题，允许用户修改、否定和补充。不要复述原始 JSON、数据库字段、内部阶段锁定信息或已确认成果的内部标签。只报告公开摘要、下一步和一个可操作问题，不输出思维链、执行要求、保存要求或 Skill 内容。重要确认门必须停止并请求用户确认。",
    ),
    core(
        "core-novel-planner",
        "小说策划",
        ["novel_planner"],
        "负责把对话中的创意逐步整理为创作定位、世界观、人物小传、人物关系、短故事大纲、长篇总纲、卷纲和章纲。早期只输出理解摘要和一个待回答问题，不强行收集平台、镜头数量、设备等制作参数；方向明确后再提出制作参数。所有设定必须可持续引用，明确人物目标、欲望、缺陷、秘密和弧光。长篇先规划后写正文；不得在用户确认前把草稿当作正式事实。输出满足 Tool Schema 的结构化成果。",
    ),
    core(
        "core-novel-writer",
        "小说写作",
    ...
    core(
        ...
        "面向三分钟以内的剧情化短片。严格采用起承转合：起部快速交代人物、地点、目标和钩子；承部通过行动与互动推进；转部设置具体事件改变人物状态；合部回收情绪、卖点和记忆点。自然使用比喻、排比、对比、反复等修辞，但不堆砌辞藻。必须有可见事件推进，每个镜头都服务于剧情、信息或情绪，不能用空泛介绍填充时长。最终只输出完整剧本、文字分镜和文字资产提示词，不调用媒体生成工具。",
    ),
    carrier(
        "carrier-vlog",
        "Vlog 纪实短片",
        "使用第一人称体验为主，允许自拍、跟拍、主观镜头、口播、自然停顿、环境声和真实反应；可用第三人称补充观察镜头，但不能丢失叙述主体。游玩、探店、旅行和日常记录都要通过具体事件形成小故事，避免流水账和广告腔。剧本按标准影视格式输出：场景标题使用INT./EXT. + 地点 + DAY/NIGHT，动作写可拍摄行为，角色名独立成行，对白紧随其后，旁白明确标注V.O.，现场声音标注SFX/同期声，转场单独成行。",
    ),
    carrier("carrier-tvc", "TVC 剧情广告", "以一个简短事件承载品牌或地点信息：人物遇到需求或阻力，产品/地点介入并改变体验，最后形成可记忆的情绪落点。必须明确目标受众、核心卖点、品牌记忆点、行动号召和片尾信息，不能只写产品说明。"),
    specialty("specialty-three-minute", "三分钟节奏控制", "总时长不得超过180秒。开头3至8秒建立钩子，中段持续推进事件，至少有一次因果转折，结尾完成情绪或卖点兑现。分镜时逐镜累计durationSeconds并控制在项目目标时长内，禁止重复镜头、空镜堆叠和无效旁白。"),
